...
drop_features = ['SaleID', 'name', 'offerType', 'seller',]
                #  'regDate', 'creatDate', 'regDate_str', 'creatDate_str', 'creatDate_str',
                #  'create_year', 'create_month', 'reg_year', 'reg_month',
                #  # 删除已编码的原始分类特征
                #  'bodyType', 'fuelType', 'gearbox', 'regionCode', 'brand_tier', 'car_age_segment',
                #  'km_segment', 'power_segment', 'reg_season']
...
for col in feature_cols:
    if X_train_full[col].isnull().sum() > 0:
        # 只对数值列计算中位数
        if X_train_full[col].dtype in ['int64', 'float64', 'int32', 'float32']:
            median_val = X_train_full[col].median()
            X_train_full[col].fillna(median_val, inplace=True)
            X_test[col].fillna(median_val, inplace=True)
        else:
            # 非数值列使用缺失字符串
            X_train_full[col] = X_train_full[col].astype(object)
            X_test[col] = X_test[col].astype(object)
            X_train_full[col].fillna('missing', inplace=True)
...
# 转换所有分类/对象列为数值
for col in X_train_full.columns:
    if X_train_full[col].dtype == 'category' or X_train_full[col].dtype == 'object':
...
output_file = 'catboost_advanced_features_result.csv'
...
iterations = range(len(evals_result['validation']['RMSE']))

# RMSE曲线
ax1.plot(iterations, evals_result['learn']['RMSE'], label='训练集', linewidth=2)
